...
class CustomerBuilding(CustomerSearch):
    ...
    connected_at: str4date = Field(validation_alias="Дата подключения")
    added_at: str4date = Field(validation_alias="Дата добавления")
    last_active_at: str4datetime = Field(validation_alias="Активность в сети")
...
class Customer(CustomerSearch):
    ...
    def validate_tariffs(cls, tariffs: dict[str, list[dict[str, str]]]):
        return [storage.tariffs[int(id["id"])] for id in tariffs.get("current", [])]
