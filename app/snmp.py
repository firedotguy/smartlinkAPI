...
from app.utils.snmp import convert_status, decode_datetime, decode_sn
...
def get_ont(olt: Olt, sn: str) -> dict | None:
    ...
    return {
        ...
        "status": convert_status(_get(session, "status", ont)),  # {
        ...
        "last_down_cause": OntLastDownCause(_get(session, "last_down_cause", ont)),
        "distance": _get(session, "distance", ont),
        # "last_dying_gasp": decode_datetime(_get(session, "last_dying_gasp", ont)),
        "eth": [
            {
                "id": i,
                "duplex": EthDuplexMode(_get(session, "eth_duplex", ont, i)),
                "speed": _get(session, "eth_speed", ont, i),
        ...
        "model": _get(session, "model", ont),
        "tx": _get(session, "tx", ont, default=0) / 100,
        "rx": _get(session, "rx", ont, default=0) / 100,
        "tx_olt": _get(session, "tx_olt", ont, add_ont_id=False, default=0) / 100,
        "rx_olt": (_get(session, "rx_olt", ont, default=10000) - 10000) / 100,
...
# print(get_ont(get_olts()[128457], "HWTCA8A37CA9"))
