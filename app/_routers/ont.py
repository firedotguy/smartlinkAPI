...
from app.ont import search_ont, reset_ont, get_ont_summary, toggle_catv
...
def api_get_ont(request: Request, olt_id: int, sn: str):
    ...
    if res[1] is not None:
        olt["name"] = res[1]
    return {"status": "success", "sn": sn, "olt": olt, "data": res[0]}
...
def api_post_ont_rewrite_sn(customer_id: int, ls: int, sn: str):
    res = api_call(
        "customer", "mark_add", f"nogi=bogi&mark_id=1&customer_id={customer_id}&_command=attach_onu&_onu_serial={sn}&_contract_number={ls}", timeout=360
    )
    if int(res["result"]):
...
@router.post("/rewrite_mac")
def api_post_rewrite_mac(customer_id: int, ls: int):
    res = api_call(
        "customer",
        "mark_add",
        f"nogi=bogi&mark_id=1&customer_id={customer_id}&_command=renew_mac_address&_userside_customer_id={customer_id}&_contract_number={ls}"
    )
    if int(res["result"]):
        return {"status": "fail", "detail": res.get("msg")}
    return {"status": "success", "message": res.get("msg")}
