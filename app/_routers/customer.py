...
from app._utils import list_to_str, to_2gis_link, to_neo_link, normalize_items, extract_sn, remove_sn, parse_agreement, status_to_str, format_mac
from app.tariff import calc_disconnect, Tariff
...
def api_get_customer_search(query: str):
    ...
    if customers:
        ...
        return {"status": "success", "customers": customers, "search_type": search_type}

    return JSONResponse({"status": "fail", "detail": "not found", "search_type": search_type}, 404)
...
def _process_customer(request_tariffs: dict[int, Tariff], request_groups: dict, customer: dict, get_olt_data: bool = True):
    ...
    will_disconnect = (
        calc_disconnect(
            ...
            customer["balance"],
            datetime.strptime(customer["date_connect"], "%Y-%m-%d")
        )
        if customer.get("date_connect")
        else None
    )
    if will_disconnect:
    ...
    return {
        ...
        "agreement": parse_agreement(customer["agreement"][0]["number"]),
        "status": status_to_str(customer["state_id"]),
        "group": {"id": list(customer["group"].values())[0]["id"], "name": request_groups[list(customer["group"].values())[0]["id"]]}
        ...
        "billing": {"id": int(customer["billing_id"]) if "billing_id" in customer and customer["billing_id"] else None, "crc": customer.get("crc_billing")},
        ...
        "address": {
            ...
            if customer.get("address", [{}])[0].get("apartment", {}).get("number")
            else None
        },
...
@router.get("/{id}")
def api_get_customer(request: Request, id: int, get_olt_data: bool = False):
    customer = api_call("customer", "get_data", f"id={id}").get("data")
    if customer is None:
        return JSONResponse({"status": "fail", "detail": "customer not found"}, 404)

    return {"status": "success", "data": _process_customer(request.app.state.tariffs, request.app.state.customer_groups, customer, get_olt_data)}
...
def api_get_customer_name(id: int):
    ...
    return {"status": "success", "id": id, "name": remove_sn(customer["full_name"])}


@router.get("")
def api_get_customers(
    request: Request, ids: str | None = None, get_data: bool = True, get_olt_data: bool = False, limit: int | None = None, skip: int | None = None
):
    customers = []
    if ids is not None:
        try:
