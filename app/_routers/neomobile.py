...
from app._utils import remove_sn, get_current_time, status_to_str
...
def neomobile_api_get_login(request: Request, phone: str, agreement: str):
    ...
    for tariff in data["tariff"]["current"]:
        if tariff["id"]:
            tariff_data.append({"id": int(tariff["id"]), "name": request.app.state.tariffs[tariff["id"]]})
    return {
        "status": "success",
        "id": id["Id"],
...
def neomobile_api_post_task(customer_id: int, phone: str, reason: str, comment: str):
    ...
    return {"status": "success", "id": id, "customer_id": customer_id, "phone": phone, "reason": reason, "comment": comment}
...
def neomobile_api_get_task(id: int):
    ...
    return {
        ...
        "comments": [
            {"id": comment["comment_id"], "content": comment["text"], "author_id": comment["employee_id"], "created_at": comment["date_add"]}
...
def neomobile_api_get_task_comment(id: int, content: str):
    ...
    return {"status": "success", "id": data["Id"], "task_id": id, "content": content}
        ...
        "data": [
            {
                ...
                "type": {
                    ...
                    "name": [
                        category["name"]
                        for category in request.app.state.tmc_categories
                        if category["id"] == [name for name in names if name["id"] == inventory["catalog_id"]][0]["inventory_section_catalog_id"]
                    ][0]
                },
