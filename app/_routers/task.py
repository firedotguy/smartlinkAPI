...
from app.api import api_call, set_additional_data
from app._utils import get_current_time, normalize_items, str_to_list, list_to_str, remove_sn
    ...
    return {
        ...
        "data": {
            ...
            "comments": [
                {
                    ...
                    "author": {
                        ...
                        "name": (
                            api_call("employee", "get_data", f"id={comment['employee_id']}").get("data", {}).get(str(comment["employee_id"]), {}).get("name")
                            if get_employee_names
                            else None
                        )
                    }
            ...
            "addata": {
                ...
                "appeal": {"phone": task["additional_data"].get("29", {}).get("value"), "type": task["additional_data"].get("28", {}).get("value")},
                "cost": float(task["additional_data"].get("26", {}).get("value")) if task["additional_data"].get("26", {}).get("value") else None
            }
            if task["type"]["id"] == 37
            else {
            ...
            "type": {"id": task["type"]["id"], "name": task["type"]["name"]},
            "author": {
                "id": task["author_employee_id"],
                "name": (
                    api_call("employee", "get_data", f"id={task['author_employee_id']}").get("data", {}).get(str(task["author_employee_id"]), {}).get("name")
                    if get_employee_names
                    else None
                )
            },
            "status": {"id": task["state"]["id"], "name": task["state"]["name"], "system_id": task["state"]["system_role"]} if task.get("state") else None,
            "address": {
            ...
            "customer": {"id": customer["id"], "name": remove_sn(customer["full_name"])} if customer else None,
...
def api_get_task_comments(id: int):
    ...
    return {
        ...
        "id": id,
        "comments": [
            {"id": comment["comment_id"], "date": comment["date_add"], "content": unescape(comment["text"]), "author": comment["employee_id"]}
...
def api_post_task(
    ...
    params = [f"work_typer={type}", f"work_datedo={get_current_time()}", f"deadline_hour=72", f"division_id={list_to_str(list_divisions)}"]
            ...
            map(
                ...
                str_to_list(
                    api_call(
                        "task",
                        "get_list",
                        f"customer_id={customer_id}&order_by=date_add&{f'&limit={limit}' if limit else ''}{f'&offset={skip}' if skip else ''}"
                    )["list"]
                )
            )
        )
            ...
            tasks_data.append(
                {
                    ...
                    "author": {
                        ...
                        .get("data", {})
                        .get(str(task["author_employee_id"]), {})
                        .get("name")
                        if get_employee_names
                        else None
                    },
