...
from app._routers.customer import _process_customer
from app._utils import normalize_items, list_to_str, str_to_list, get_coordinates, get_box_map_link

router = APIRouter(prefix="/box")


@router.get("/{id}")
def api_get_box(request: Request, id: int, get_olt_data: bool = False, get_tasks: bool = False, limit: int | None = None, exclude_customer_ids: str = "[]"):
    ...
    if customer_ids:
        ...
        raw_customers = normalize_items(api_call("customer", "get_data", f"id={list_to_str(fetch_customer_ids)}"))
