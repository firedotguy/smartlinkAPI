...
def get_task(id: int) -> Task | None:
    ...
    return Task.model_validate(res, context=res)


def get_task_ids(customer_id: int) -> list[int]:
    l.info("get task ids customer_id=%s", customer_id)
    return list(map(int, api_call("task", "get_list", customer_id=customer_id, order_by="date_add")["list"].split(",")))


def get_tasks(*ids: int) -> list[Task]:
    l.info("get tasks ids=%s", ids)
    tasks = api_call("task", "show", id=",".join(map(str, ids)))["data"]
    if len(ids) > 1:
        tasks = dict2list_validator(tasks)

        if "error" in tasks:
            l.error("some task(s) not found")
        return [Task.model_validate(task, context=task) for task in tasks if task != "error"]

    return [Task.model_validate(tasks, context=tasks)]
