...
from app.config import SSH_USER, SSH_PASSWORD
from app._utils import format_mac
...
def get_ont_summary(host: str, interface: dict) -> dict:
    ...
    try:
        ...
        channel.close()
        ssh.close()
        return {"status": "success", "online": online, "offline": offline, "onts": onts}
...
def _parse_output(raw: str) -> tuple[dict, list[list[dict]]]:
    ...
    while i < len(divider_indices) - 1:
        ...
        is_heading = heading_lines and any(search(r"\s{3,}", line) for line in heading_lines) and not any(":" in line for line in heading_lines)
        ...
        try:
            dat = table_read(table_text, reader=FixedWidth, outputter=DictOutputter, header_start=0, data_start=data_start_idx, delimiter=" ", numpy=False)
...
def _parse_optical_info(raw: str) -> dict:
    ...
    return {
        ...
        "vendor": {"name": data.get("Vendor name"), "rev": data.get("Vendor rev"), "pn": data.get("Vendor PN"), "sn": data.get("Vendor SN")}
    }
...
def _parse_service_port(raw: str, interface: dict) -> int | None:
    raw = raw.replace(
        f"{interface['fibre']}/{interface['service']} /{interface['port']}", f"{interface['fibre']}/{interface['service']}/{interface['port']}"
    )  # change F/S /P -> F/ S/ P
