...
from app.db.models import Employee, Ont
...
def get_employee_name(db: Session, id: int) -> str | None:
    l.info("get employee name id=%s", id)
    employee = db.query(Employee).where(Employee.us_id == id).first()
    if employee:
    ...
    employee = api_get_employee(id)
    if employee:
        db.add(Employee(us_id=id, role=employee.role, username=employee.username, name=employee.name))
        db.commit()
        return employee.name
    l.error("employee not found")
