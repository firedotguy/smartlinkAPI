...
def setup_logging(level: str = "INFO", colorful: bool = True) -> logging.Logger:
    ...
    logger = logging.getLogger("smartlinkapi")
...
def format_request(request: Request, response: Response, time: float, colorful: bool = True) -> str:
    if colorful:
        ...
        return f"{request.method} {request.url.path} -> [{color}]{response.status_code}[/] in {time}s"
    return f"{request.method} {request.url.path} -> {response.status_code} in {time}s"
