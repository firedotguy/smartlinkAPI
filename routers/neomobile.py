...
def neomobile_api_post_task(customer_id: int, phone: str, reason: str, comment: str):
    id = api_call('task', 'add', f'work_typer=37&work_datedo={get_current_time()}&customer_id={customer_id}&author_employee_id=184&opis={comment}&deadline_hour=72&employee_id=184&division_id=81')['Id']
        ...
        'data': [
            {
                ...
                'type': {
                    ...
                    'name': [category['name'] for category in request.app.state.tmc_categories if category['id'] == [name for name in names if name['id'] == inventory['catalog_id']][0]['inventory_section_catalog_id']][0]
                },
